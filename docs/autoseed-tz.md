...
1. Если локальное время политики попадает в диапазон `23:00–08:00`, целевым считается `nightPreferredServerId`.
...
4. Сначала проверяется порядок `1 -> 2 -> 4`.
...
- предпочтительный формат: `steam://connect/IP:PORT`.
